# 🩸 CHARACTER SHEET: RASHUK (LAXAS LANGLEY)
> **Active Campaign:** Blightborne: The Ashen Trail  
> **Class & Level:** Fighter (Battle Master Archetype) 5  
> **Heritage / Species:** Half-Demon / Half-Human Hybrid (Abyssal Bloodline mechanics)  
> **Background:** Soldier (Origin Feat: Savage Attacker)  
> **Alignment:** **Lawful Evil** *(Cold, calculating retribution; power is the only law)*  
> **Speed:** 30 ft | **Proficiency Bonus (PB):** +3  
> **Hit Points:** 59 / 59 (Hit Dice: 5d10; 10 + 4d10 + 15 Con + 5 Abyssal Fortitude)  
> **Armor Class:** 18 (Plate Armor) | **Initiative:** +1  
> **Maneuver DC:** 15 (8 + 3 PB + 4 STR) | **Superiority Dice:** 4d8 / Short Rest

---

## 📊 ABILITY SCORES & SAVING THROWS
* **STR:** 18 (+4) ➔ **Save: +7 ★ (Proficient)** *(Point Buy 15 + 2 Background + 1 GWM Feat)*
* **DEX:** 13 (+1) ➔ **Save: +1**
* **CON:** 16 (+3) ➔ **Save: +6 ★ (Proficient)** *(Point Buy 15 + 1 Background)*
* **INT:** 8 (-1) ➔ **Save: -1**
* **WIS:** 12 (+1) ➔ **Save: +1**
* **CHA:** 8 (-1) ➔ **Save: -1**

---

## 🎯 SKILLS & PROFICIENCIES
* **Proficient Skills:**
  * 🏃 **Athletics (STR):** +7 ★
  * 😠 **Intimidation (CHA):** +2 ★ *(Half-Demon Demeanor / Soldier)*
  * 👁️ **Perception (WIS):** +4 ★
  * 🌲 **Survival (WIS):** +4 ★
* **Untrained Skills:**
  * Acrobatics (DEX): +1 | Animal Handling (WIS): +1 | Arcana (INT): -1 | Deception (CHA): -1 | History (INT): -1 | Insight (WIS): +1 | Investigation (INT): -1 | Medicine (WIS): +1 | Nature (INT): -1 | Performance (CHA): -1 | Persuasion (CHA): -1 | Religion (INT): -1 | Sleight of Hand (DEX): +1 | Stealth (DEX): +1
* **Armor & Weapon Proficiencies:** All armor (Light, Medium, Heavy, Shields), Simple weapons, Martial weapons.
* **Tool Proficiencies:** Smith’s Tools (via Student of War), Playing Card Set, Vehicles (Land).
* **Languages:** Common, Abyssal.

---

## ⚔️ ATTACKS & COMBAT ACTIONS (EXTRA ATTACK x2)

* 🪓 **Overseer Vhol's Obsidian Greatcleaver (Primary Equipped):**
  * **To Hit:** +7 (+4 STR, +3 PB)
  * **Damage:** `2d6 + 4` Slashing (Great Weapon Fighting: 1s & 2s count as 3s).
  * **Weapon Mastery (Graze):** On a missed attack roll, deal **4 flat Slashing damage** (STR mod).
  * **Special Ability — Laceration (Deep Bleed):** On a critical hit or natural 18–20 attack roll, target suffers deep arterial lacerations dealing **1d4 ongoing bleed/necrotic damage** at the start of each of its turns until treated (DC 13 Medicine check or magical healing).
  * **Great Weapon Master:** Add **+3 damage** (PB) once per turn on hit; Crits or kills grant a Bonus Action melee attack.

* 🗡️ **Greatsword (Secondary Arsenal):**
  * **To Hit:** +7 | **Damage:** `2d6 + 4` Slashing (GWF active, Graze Mastery).

* 🪓 **Halberd (Reach Arsenal):**
  * **To Hit:** +7 | **Damage:** `1d10 + 4` Slashing (Reach 10 ft, Heavy, Two-Handed; Cleave Mastery).

* 🏹 **Heavy Crossbow (Ranged Arsenal):**
  * **To Hit:** +4 (+1 DEX, +3 PB) | **Damage:** `1d10 + 1` Piercing (Range 100/400 ft, Push Mastery 10 ft).

* 👊 **Unarmed Strike / Demon Claws:**
  * **To Hit:** +7 | **Damage:** `5` flat Bludgeoning or `1d4 + 4` Slashing.

---

## 🎲 BATTLE MASTER MANEUVERS (4 KNOWN • 4d8 SUPERIORITY POOL)
* **Trip Attack:** On weapon hit, expend 1 die to add **+1d8 damage**. Large or smaller target must succeed on a **DC 15 STR Save** or fall **Prone**.
* **Riposte:** When an enemy misses you with a melee attack, spend your **Reaction** and 1 die to strike back (+7 Hit); on hit, add **+1d8 damage**.
* **Precision Attack:** Expend 1 die to add **+1d8 to an attack roll** before or after seeing the roll.
* **Menacing Attack:** On weapon hit, expend 1 die to add **+1d8 damage**. Target must succeed on a **DC 15 WIS Save** or become **Frightened** of you until the end of your next turn.

---

## ⚡ FIGHTER CLASS FEATURES & FEATS
* **Extra Attack:** Attack twice per Attack action.
* **Action Surge (1/Short or Long Rest):** Gain 1 additional action on your turn.
* **Second Wind (2/Short or Long Rest):** Bonus Action to regain `1d10 + 5` HP.
* **Tactical Mind:** When failing an ability check, expend a Second Wind use to add `+1d10` to the roll; if you still fail, the use is refunded.
* **Fighting Style — Great Weapon Fighting:** When rolling damage with a two-handed melee weapon, any 1 or 2 counts as a 3.
* **Origin Feat (Soldier) — Savage Attacker:** Once per turn on a hit, roll the weapon's damage dice twice and keep the higher result.
* **Level 4 Feat — Great Weapon Master:** +1 STR; adds +3 PB damage once per turn; bonus action attack on crits/kills.

---

## 🩸 HYBRID HERITAGE & ABYSSAL BLOODLINE
* **Darkvision:** 60 ft.
* **Fiendish Resilience:** Resistance to Poison damage; Advantage on saves against the Poisoned condition.
* **Abyssal Fortitude:** Maximum HP increased by +1 per Fighter level (+5 total).
* **Otherworldly Presence:** Knows the *Thaumaturgy* cantrip (Charisma-based).
* **Innate Spells:**
  * *Poison Spray* (Cantrip; 30 ft range, DC 10 Con Save or 1d12 poison dmg).
  * *Ray of Sickness* (1st-level; 1 free cast/Long Rest; +2 spell attack, 2d8 poison dmg + DC 10 Con Save vs Poisoned).
* ⚠️ **Blight-Born Blood Hazard:** Rashuk's blood is vibrant human red. Requires **DC 12–14 check** (Sleight of Hand / Athletics) to conceal or wipe away when bleeding near demon sentries or braziers.

---

## 🎒 INVENTORY & ARSENAL
* **Equipped Armor:** 🛡️ Plate Armor (18 AC)
* **Equipped Weapons:** 
  * 🪓 **Overseer Vhol's Obsidian Greatcleaver** *(Equipped — 2d6+4 Slash + Laceration)*
  * 🗡️ Greatsword
  * 🪓 Halberd
  * 🏹 Heavy Crossbow
* **Worn Relics & Key Quest Items:**
  * 💍 **Inquisitorial Signet Ring** *(Worn — Authorizes passage & commandeers regional lockboxes)*
  * 🗺️ **Grand Tithe Supply Map to Zul-Gaza** *(Details 3 supply fortresses)*
  * 🔑 **Obsidian Vault Master Key** *(Unlocks Fort Kar-Vash gates & armories)*
  * 🎯 20x Crossbow Bolts
  * 🍷 1x Ghost-Poppy Salve *(Restores 2d6+1 HP, suppresses pain/fear)*
  * 🧪 1x Alchemical Quenching Dust *(Blinds 15ft for 1 rd or douses braziers)*
  * 🎒 Explorer's Pack
  * 🎖️ Soldier Insignia (Sanctuary / Mercenary Rank)
  * 🃏 Playing Card Set
  * 🔨 Smith's Tools
* **Wealth:** **15 GP, 0 SP** (10 SP = 1 GP)
